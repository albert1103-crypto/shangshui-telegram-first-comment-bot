...
export default {async fetch(request,env){const u=new URL(request.url);if(request.method==="GET"&&u.pathname==="/")return new Response(UI,{headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store"}});if(u.pathname==="/health"||u.pathname.startsWith("/api/"))return proxy(request,env);return j({error:"Not found"},404)},async scheduled(_c,env,ctx){ctx.waitUntil(stub(env).fetch("https://state.internal/api/tick",{method:"POST"}))}};
